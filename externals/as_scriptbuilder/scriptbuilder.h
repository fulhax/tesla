...
typedef int (*INCLUDECALLBACK_t)(const char *include, const char *from,
...
public:
    ...
    const char *GetMetadataStringForTypeMethod(int typeId,
...
protected:
    ...
    struct SMetadataDecl {
        SMetadataDecl(
            std::string m, std::string d, int t, std::string c,
            std::string ns) : metadata(m), declaration(d), type(t),
            parentClass(c), nameSpace(ns) {}
