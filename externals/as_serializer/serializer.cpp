...
int CSerializer::Store(asIScriptModule *mod)
{
    ...
    for (i = 0; i < mod->GetGlobalVarCount(); i++) {
        ...
        m_root.m_children.push_back(new CSerializedValue(&m_root, name, nameSpace,
                                    mod->GetAddressOfGlobalVar(i), typeId));
    }

    // Second store extra objects
    for (i = 0; i < m_extraObjects.size(); i++) {
        m_root.m_children.push_back(new CSerializedValue(&m_root, "", "",
                                    m_extraObjects[i].originalObject, m_extraObjects[i].originalTypeId));
    }
...
CSerializedValue::CSerializedValue(CSerializedValue *parent,
                                   const std::string &name, const std::string &nameSpace, void *ref, int typeId)
{
    Init();
...
void CSerializedValue::ClearChildren()
{
    ...
    if ((m_typeId & asTYPEID_OBJHANDLE) && m_children.size() == 1 &&
        m_children[0]->m_restorePtr) {
        m_serializer->m_engine->ReleaseScriptObject(
            m_children[0]->m_restorePtr,
            GetTypeInfoFromId(m_children[0]->m_typeId)
        );
    }
...
void CSerializedValue::Store(void *ref, int typeId)
{
    ...
    if (m_typeId & asTYPEID_OBJHANDLE) {
        ...
        for (asUINT i = 0; i < type->GetPropertyCount(); i++) {
            ...
            m_children.push_back(new CSerializedValue(this, childName, "",
                                 obj->GetAddressOfProperty(i), childId));
        }
    } else {
...
void CSerializedValue::Restore(void *ref, int typeId)
{
    ...
    if (typeId <= asTYPEID_DOUBLE && typeId != m_typeId) {
        return;    // TODO: We may try to do a type conversion for primitives
    }

    if ((typeId & ~asTYPEID_MASK_SEQNBR) ^ (m_typeId & ~asTYPEID_MASK_SEQNBR)) {
        return;
    }

    asITypeInfo *type = m_serializer->m_engine->GetTypeInfoById(typeId);

    if (type && m_typeName != type->GetName()) {
        return;
    }
...
void CSerializedValue::ReplaceHandles()
{
    if (m_handlePtr) {
        ...
        if (handle_to == 0) {
            ...
            asITypeInfo *type = GetType();
            CSerializedValue *need_create = new CSerializedValue(this, m_name,
                    m_nameSpace, m_handlePtr, type->GetTypeId());
        ...
        if (m_handlePtr) {
            ...
            if (m_restorePtr && handleTo && handleTo->m_restorePtr) {
                ...
                *(void **)m_restorePtr = handleTo->m_restorePtr;
                ...
                m_serializer->m_engine->AddRefScriptObject(
                    handleTo->m_restorePtr,
                    GetTypeInfoFromId(m_typeId)
                );
            }
        } else {
...
asITypeInfo* CSerializedValue::GetTypeInfoFromId(int typeId)
{
    asITypeInfo *type = m_serializer->m_engine->GetTypeInfoById(typeId);
    return type;
}
